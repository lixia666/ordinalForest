...
#' true values.\cr
#' 2) Calculate the approximated optimal score set s_1,...,s_J as a particular summary over the \code{nbest} best score sets generated in 1),
...
#'
#' Prediction is performed by majority voting of the predictions of the individual trees in the OF.
#'
#' OF features a permutation variable importance measure that considers the misclassification error as error function.
#' }
#'
...
ordfor <-
  function(depvar, data, nsets=1000, ntreeperdiv=100, ntreefinal=5000, perffunction = c("equal", "proportional", "oneclass", "custom"),
    ...
    if(!naive) {
      ...
      bordersbest <- c(0, colMeans(bordersb[order(perffunctionvalues, decreasing=TRUE)[1:nbest],][,c(-1,-ncol(bordersb))]), 1)
